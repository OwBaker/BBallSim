...
class Match:
    ...
    def possession(self, team):
        '''
        whoever has the ball, decide what to do
        repeat until possession ends
        '''
...
The Match Class:
    - values:
        team 1 - team 1 object
        team 2 - team 2 object
        t1score - team 1 score
        t2score - team 2 score
        currentq - stores current quarter
        gametime (capped at 48 mins) - tracks total game time
        qtime - tracks quarter time
...
The Player Class:
    - values:
        hasball - boolean to show if player has ball
        coords - coordinates of player location on court
            sidenote on this - player location is stored as coordinates on a 188 by 100 grid

    - methods:
        pass(teammate) - attempts to pass ball to target
        move(target) - attempts to dribble to target location, same thing as driving
        shoot() - attempts a shot from current location
...
def decideOutcome(t1, t2):
    ...
    # team1 values for eoffense and defense
    team1offense = 0.0
    team1defense = 0.0

    # for each player, calculate the game value of each of their stats
    for i in range(len(t1.players["PlayerID"]) - 1):
        ...
        # add the player's value to the team's total sum
...
def game_val(value):
    rolls = []
    for i in range(3):
        rolls.append(rand.uniform(0.4, 1)) # get 3 random numbers between 0.4 and 1
    finalroll = max(rolls) # choose highest of 3 rolls
    gameval = (value * finalroll) # multiply by raw stat to get game value
    return gameval
...
def initializeTeams(t1, t2):
    ...
    for i in range(5):
        ...
        # add to team 1
        t1.players = pd.concat([t1.players, playertoadd])
...
def save_teams(teams):
    ...
    team.players = pd.read_json(StringIO(team.players))
...
def plot_wins(t1, t2):
    ...
    plt.bar([f"{t1.name} Wins", f"{t2.name} Wins"], [t1wins, t2wins])
    plt.show()
