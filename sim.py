...
def decideOutcome(t1, t2):
    ...
    for i in range(len(t1.players["PlayerID"]) - 1):
        playeroffsum = (game_val(t1.players["InShot"][i]) + game_val(t1.players["MidShot"][i]) + game_val(t1.players["ThreeShot"][i])) 
        playerdefsum = (game_val(t1.players["InDef"][i]) + game_val(t1.players["MidDef"][i]) + game_val(t1.players["PerDef"][i]))
    ...
    for i in range(len(t2.players["PlayerID"]) - 1):
        playeroffsum = (game_val(t2.players["InShot"][i]) + game_val(t2.players["MidShot"][i]) + game_val(t2.players["ThreeShot"][i])) 
        playerdefsum = (game_val(t2.players["InDef"][i]) + game_val(t2.players["MidDef"][i]) + game_val(t2.players["PerDef"][i]))

        team2offense += playeroffsum
        team2defense += playerdefsum
    
    # compare and decide final outcome
    print(f"Team One Values: {team1offense, team1defense} \nTeam Two Values: {team2offense, team2defense}")
...
def play(team1, team2):
    ...
    print()
    print(f"The {team1.name}: {team1.players['Name']}")
    print(f"The {team2.name}: {team2.players['Name']}")
...
def gen_players(amount):
    ...
    for i in range(amount):
        ...
        dict["InShot"].append(rand.randint(70, 100))
        dict["MidShot"].append(rand.randint(70, 100))
        dict["ThreeShot"].append(rand.randint(70, 100))
        dict["InDef"].append(rand.randint(70, 100))
        dict["MidDef"].append(rand.randint(70, 100))
        dict["PerDef"].append(rand.randint(70, 100))
...
def initializeTeams(t1, t2):
    ...
    for i in range(5):
        ...
        t1.players["InShot"].append(players['InShot'][playertoadd])
        t1.players["MidShot"].append(players['MidShot'][playertoadd])
        t1.players["ThreeShot"].append(players['ThreeShot'][playertoadd])
        t1.players["InDef"].append(players['InDef'][playertoadd])
        t1.players["MidDef"].append(players['MidDef'][playertoadd])
        t1.players["PerDef"].append(players['PerDef'][playertoadd])
    ...
    for i in range(5):
        ...
        t2.players["InShot"].append(players['InShot'][playertoadd])
        t2.players["MidShot"].append(players['MidShot'][playertoadd])
        t2.players["ThreeShot"].append(players['ThreeShot'][playertoadd])
        t2.players["InDef"].append(players['InDef'][playertoadd])
...
1. Play Match
- plays game and shows winner
- allows user to exit back to main menu

2. View Teams
- prints data for both teams
- allows exit to main screen
