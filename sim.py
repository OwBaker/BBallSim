...
class Team:
    ...
    def __init__(self, name, player_count):
        self.name = name
        self.players = gen_players(player_count)

# decides a game's outcome given the dictionaries for each team, returns name of victor and prints some debug info for now
def decideOutcome(t1, t2):

    # team1 values
    team1offense = 0.0
    team1defense = 0.0
    for i in range(len(t1.players["PlayerID"]) - 1):
        playeroffsum = (game_val(t1.players.loc[i, "InShot"]) + game_val(t1.players.loc[i, "MidShot"]) + game_val(t1.players.loc[i, "ThreeShot"])) 
        playerdefsum = (game_val(t1.players.loc[i, "InDef"]) + game_val(t1.players.loc[i, "MidDef"]) + game_val(t1.players.loc[i, "PerDef"]))

        team1offense += playeroffsum
        team1defense += playerdefsum
    ...
    for i in range(len(t2.players["PlayerID"]) - 1):
        ...
        team2offense += playeroffsum
        team2defense += playerdefsum
    
    # compare and decide final outcome
    ...
    print(f"{t2.name} PointDif: {team2pointdif}")


    if team1pointdif > team2pointdif:
        return t1.name
    elif team1pointdif < team2pointdif:
        return t2.name
    else:
        t1crunch = game_val(team1pointdif)
        t2crunch = game_val(team2pointdif)
        if t1crunch > t2crunch:
            return t1.name
        elif t1crunch < t2crunch:
            return t2.name
        else:
...
# calculates the in game value of a given statistic of a given player   
def game_val(value):
    rolls = []
    for i in range(3):
        rolls.append(rand.uniform(0.4, 1))
    finalroll = max(rolls)
    gameval = (value * finalroll)
    return gameval
...
def gen_players(amount):
    ...
    dict = pd.DataFrame(columns=["PlayerID", "Name", "InShot", "MidShot", "ThreeShot", "InDef", "MidDef", "PerDef"],
    index=[0])

    for i in range(amount):
        dict.loc[i, "PlayerID"] = i
        dict.loc[i, "Name"] = names[rand.randint(0, len(names) - 1)]
        dict.loc[i, "InShot"] = rand.randint(70, 100)
        dict.loc[i, "MidShot"] = rand.randint(70, 100)
        dict.loc[i, "ThreeShot"] = rand.randint(70, 100)
...
# displays raw data of teams
def detail(team):
    data = team.players

    print(f"The {team.name} Player Stats: \n---------------")
    print(data)
    print('---------------\n')
...
def initializeTeams(t1, t2):
    ...
    players = gen_players(10)

    for i in range(5):
        playertoadd = players.loc[(rand.randint(0, len(players["PlayerID"]) - 1)), "PlayerID"]
        while playertoadd in t1.players['PlayerID'].values or playertoadd in t2.players['PlayerID'].values:
            playertoadd = players.loc[(rand.randint(0, len(players["PlayerID"]) - 1)), "PlayerID"]

        t1.players.loc[i, "PlayerID"] = playertoadd
        t1.players.loc[i, "Name"] = players.loc[playertoadd, "Name"]
        t1.players.loc[i, "InShot"] = players.loc[playertoadd, "InShot"]
        t1.players.loc[i, "MidShot"] = players.loc[playertoadd, "MidShot"]
        t1.players.loc[i, "ThreeShot"] = players.loc[playertoadd, "ThreeShot"]
        t1.players.loc[i, "InDef"] = players.loc[playertoadd, "InDef"]
        t1.players.loc[i, "MidDef"] = players.loc[playertoadd, "MidDef"]
        t1.players.loc[i, "PerDef"] = players.loc[playertoadd, "PerDef"]
        
    
    for i in range(5):
        playertoadd = players.loc[(rand.randint(0, len(players["PlayerID"]) - 1)), "PlayerID"]
        while playertoadd in t2.players["PlayerID"].values or playertoadd in t1.players["PlayerID"].values:
            playertoadd = players.loc[(rand.randint(0, len(players["PlayerID"]) - 1)), "PlayerID"]

        t2.players.loc[i, "PlayerID"] = playertoadd
        t2.players.loc[i, "Name"] = players.loc[playertoadd, "Name"]
        t2.players.loc[i, "InShot"] = players.loc[playertoadd, "InShot"]
        t2.players.loc[i, "MidShot"] = players.loc[playertoadd, "MidShot"]
        t2.players.loc[i, "ThreeShot"] = players.loc[playertoadd, "ThreeShot"]
        t2.players.loc[i, "InDef"] = players.loc[playertoadd, "InDef"]
        t2.players.loc[i, "MidDef"] = players.loc[playertoadd, "MidDef"]
        t2.players.loc[i, "PerDef"] = players.loc[playertoadd, "PerDef"]
